...
- `FMS_STATE_FILE` 기반 런타임 상태 저장 및 재시작 복구
- Mission 단계 상태 머신: source 이동, handshaking, target 이동, 완료/충전 전환
...
상태 저장 파일은 기본적으로 `data/fms-state.json`에 생성됩니다. 테스트에서는 자동으로 비활성화되며, 운영 시에는 `FMS_STATE_FILE=/data/fms-state.json`처럼 원하는 위치를 지정할 수 있습니다.
...
- compose 실행 시 `fms-back-state` Docker volume에 backend 상태가 저장되어 컨테이너 재시작 후에도 로봇/Task/Mission/Event 상태가 유지됩니다.
